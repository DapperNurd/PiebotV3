    ...
    async execute(interaction, client, promisePool) {
        ...
        if(subCommandGroup == "youtube") {
            ...
            if(subCommand == "create") {
                ...
                if(rows.length >= 5) {
                    return await interaction.editReply({ content: "You can only have up to 5 youtube listeners.", ephemeral: true });
                }
                ...
                if( response.data.pageInfo.totalResults <= 0 ) {
                    return await interaction.editReply({ content: "This channel ID does not exist or is invalid. Please check your spelling and try again.", ephemeral: true });
                }
                ...
                if(existingRows.length > 0) {
                    return await interaction.editReply({ content: "You already have created this listener.", ephemeral: true });
                }
                ...
                if(existingRows.length === 0) {
                    return await interaction.editReply({ content: "You do not have this listener created. Make sure everything is spelled correctly.", ephemeral: true });
                }
                ...
                return await interaction.editReply({ content: `You have successfully removed the youtube listener for channel: https://www.youtube.com/channel/${youtubeChannelID} in <#${discordChannelID}>.`, ephemeral: true });
            }
                ...
                if(rows.length === 0) {
                    return await interaction.editReply({ content: "You have no existing youtube listeners.", ephemeral: true });
                }
            ...
            return await interaction.editReply({ content: "Invalid subcommand group.", ephemeral: true });
        }
    }
}